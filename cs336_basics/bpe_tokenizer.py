...
class BPETokenizer():
    ...
    def train(self, input_path: str | os.PathLike):
        ...
        while self.next_token_id < self.vocab_size:
            pair_counts = self.get_pair_counts(byte_pretoken_counts)
            most_frequent_pair = self.get_most_frequent_pair(pair_counts)
            self.create_new_token_and_update_vocab_and_merges(most_frequent_pair)
            byte_pretoken_counts = self.update_byte_pretoken_counts(byte_pretoken_counts, most_frequent_pair)
            self.next_token_id += 1
        
        return self.vocab, self.token_merges


    def get_pair_counts(self, byte_pretoken_counts: Counter) -> Counter:
    ...
    def get_most_frequent_pair(self, pair_counts: Counter) -> tuple[int, int]:
    ...
    def create_new_token_and_update_vocab_and_merges(self, most_frequent_pair: tuple[int, int]) -> None:
    ...
    def update_byte_pretoken_counts(self, byte_pretoken_counts: Counter, most_frequent_pair: tuple[int, int]) -> Counter:
